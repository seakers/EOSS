...
import mining.Apriori.AprioriFeature;
...
//    	System.out.println("...Starting Apriori");
        ...
        Collections.sort(this.drivingFeatures, new FeatureComparator(FeatureMetric.FCONFIDENCE));
        ...
        System.out.println("...[DrivingFeatures] Number of features before mRMR: " + reduced_set.size() + ", with max lift of " + reduced_set.get(0).getLift());
    ...
    public ArrayList<DrivingFeature> getPresetDrivingFeatures() {
        ...
        try {
            ...
            for (int i : addedFeatureIndices) {
                double support = featureData_metrics.get(i)[0];
                double lift = featureData_metrics.get(i)[1];
                double fconfidence = featureData_metrics.get(i)[2];
                double rcondfidence = featureData_metrics.get(i)[3];
                
                this.presetDrivingFeatures.add(new DrivingFeature(id, featureData_name.get(i), featureData_exp.get(i), support, lift, fconfidence, rcondfidence));
    ...
    public ArrayList<DrivingFeature> getDrivingFeatures() {
        ...
        ArrayList<AprioriFeature> new_features = ap.runApriori(this.maxLength);

        // Create a new list of driving features (assign new IDs)
        int id = 0;
        for (int f = 0; f < new_features.size(); f++) {

            CompoundFeature  feat = new_features.get(f);
            String expression = "";
            String name = "";
            Collection <Integer> featureIndices = feat.getElements();
            ...
            DrivingFeature df = new DrivingFeature(id, name, expression, 
                    feat.getSupport(),feat.getLift(),feat.getFConfidence(), feat.getRConfidence());
    ...
    public void recordMetaInfo(PrintWriter w, DrivingFeature feature) {
        ...
        try {
            ...
            w.print("/" + feature.getSupport() + "/" + feature.getLift() + "/" + feature.getFConfidence() + "// " + name + "\n");
