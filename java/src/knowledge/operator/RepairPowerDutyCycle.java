...
public class RepairPowerDutyCycle extends AbstractEOSSOperator {
    ...
    /**
     * The power rate duty cycle that a spacecraft must be at or higher
     */
    ...
    public RepairPowerDutyCycle(double threshold, int numModifications) {
    ...
    protected InstrumentAssignmentArchitecture evolve(InstrumentAssignmentArchitecture child) {
        ...
        for (String name : child.getMissionNames()) {
            ...
            for (Spacecraft s : mission.getSpacecraft().keySet()) {
                if(checkPower(copy, mission.getSpacecraft().get(s), threshold)){
        ...
        for (int i = 0; i < numModifications; i++) {
            ...
            while (!checkPower(copy, change, threshold)) {
     ...
     * @return 
     */
    private double computePowerDutyCycle(double dataRatePerOrbit){
     ...
     * @return 
     */
    private boolean checkPower(InstrumentAssignmentArchitecture arch, Orbit orbit, double threshold){
        ...
        return computePowerDutyCycle(dataRatePerOrbit) > threshold;
    }
