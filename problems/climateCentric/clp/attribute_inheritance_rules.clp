...
(defrule MANIFEST2::assert-manifested-instruments
    ...
    ?miss <- (MANIFEST::Mission (Name ?name)  (num-of-sats-per-plane# ?nsats) (orbit-altitude# ?h) (orbit-inclination ?inc) (instruments $?list-of-instruments))
    
       =>
    (foreach ?x $?list-of-instruments (assert (CAPABILITIES::Manifested-instrument (Name ?x) (flies-in ?name) (num-of-sats-per-plane# ?nsats) (orbit-altitude# ?h) (orbit-inclination ?inc) )))
...
(defrule MANIFEST::get-instrument-revisit-times-from-database
    ...
    ?sub <- (DATABASE::Revisit-time-of  (num-of-sats-per-plane# ?nsats) 
        (orbit-altitude# ?h) (orbit-inclination ?inc) 
...
(defrule MANIFEST::compute-fov-from-angular-res-and-npixels-azimuth 
    ...
    =>
	(bind ?fov (* ?ara ?npix)); 
    (modify ?instr (Field-of-view-azimuth# ?fov) )
    )
...
(defrule MANIFEST::compute-revisit-time
    ...
    ?sub1 <- (CAPABILITIES::Manifested-instrument (Name ?ins) (num-of-sats-per-plane# ?ns)
         (orbit-altitude# ?h) (orbit-inclination ?inc) (Field-of-view# ?fov) )
    
    ?sub2 <- (DATABASE::Revisit-time-of (num-of-sats-per-plane# ?ns) (orbit-altitude# ?h)
