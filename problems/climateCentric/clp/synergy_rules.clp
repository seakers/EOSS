...
(defrule SYNERGY::spatial-disaggregation "A frequent coarse spatial resolution measurement can be combined with a sparse high spatial resolution measurement to produce a frequent high spatial resolution measurement with average accuracy"
    ...
    (SYNERGY::cross-registered (measurements $?m))
    (test (member$ ?id1 $?m))
    (test (member$ ?id2 $?m))
...
(defrule SYNERGY::spatial-disaggregation-hyperspectral "A hyperspectral coarse spatial resolution measurement can be combined with a multispectral high spatial resolution measurement to produce a high spatial resolution hyperspectral measurement with lower accuracy"
...
(defrule SYNERGY::spatial-averaging "any image can be averaged out in space to provide a new, better accuracy, coarser resolution image"
...
(defrule SYNERGY::time-averaging "any image can be averaged out in time to provide a new, better accuracy, sparser temporal resolution image"
...
(defrule SYNERGY::seafloor-topography 
...
(defrule SYNERGY::river-plumes-from-ocean-color "River plumes and sediment fluxes can be measured with an ocean color measurement of sufficient horizontal spatial resolution"
        ?oc <- (REQUIREMENTS::Measurement (Parameter "3.1.1 Ocean color - 410-680nm (Chlorophyll absorption and fluorescence, pigments, phytoplankton, CDOM)") 
			(Horizontal-Spatial-Resolution ?hsr &~nil) 
			(Id ?id1) (Temporal-resolution ?tr &~nil))
    (test (>= (SameOrBetter Horizontal-Spatial-Resolution ?hsr High-10-100m) 0))
    (test (>= (SameOrBetter Temporal-resolution ?tr High-12h-24h) 0))
    ?sub <- (REQUIREMENTS::Measurement (Parameter "3.1.2 Extended ocean color - UV (enhanced DOC, CDOM)") (Horizontal-Spatial-Resolution ?hsr &~nil) (Id ?id2))
    (SYNERGY::cross-registered (measurements $?m))
    (test (member$ ?id1 $?m))
    (test (member$ ?id2 $?m))
    =>
...
(defrule SYNERGY::hydrocarbon-reservoir-monitoring-from-surface-deformation "Hydrocarbon reservoirs can be monitored by measuring surface deformation and surface composition"
        ?sd <- (REQUIREMENTS::Measurement (Parameter "2.2.1 surface deformation") (Horizontal-Spatial-Resolution ?hsr &~nil) (Id ?id1))
    (test (>= (SameOrBetter Horizontal-Spatial-Resolution ?hsr High-10-100m) 0))
    ?sub <-(REQUIREMENTS::Measurement (Parameter "2.6.5 surface composition") (Horizontal-Spatial-Resolution ?hsr &~nil) (Id ?id2))
    (SYNERGY::cross-registered (measurements $?m))
    (test (member$ ?id1 $?m))
    (test (member$ ?id2 $?m))
    =>
...
(defrule SYNERGY::flood-monitoring-from-hires-topography "Hi res topography with 5m horizontal spatial resolution and 10cm accuracy allows flood monitoring"
        ?topo <-  (REQUIREMENTS::Measurement (Parameter "2.2.2 Hi-res topography") (Horizontal-Spatial-Resolution ?hsr & :(neq ?hsr nil)) (Id ?id1) )
    (test (>= (SameOrBetter Horizontal-Spatial-Resolution ?hsr Very-high-1-10m) 0))
    =>
...
(defrule SYNERGY::groundwater-storage-from-gravity "Groundwater storage can be inferred from precise gravity measurements"
...
(defrule SYNERGY::glacier-mass-balance-from-gravity "Glacier mass balance measurements can be inferred from precise gravity measurements using ice topography measurements"
...
(defrule SYNERGY::ocean-mass-distribution-from-gravity "Ocean mass distribution can be inferred from precise gravity measurements"
        ?grav <- (REQUIREMENTS::Measurement (Parameter "5.1.1 Geoid and gravity field variations") (Horizontal-Spatial-Resolution ?hsr & :(neq ?hsr nil)) (Id ?id1))
    =>
...
(defrule SYNERGY::visible-atmospheric-plume-from-aerosols "Visible atmospheric plumes can be measured from high temporal and spatial resolution multispectral aerosol measurements"
...
(defrule SYNERGY::black-carbon-from-aerosols "Black carbon can be measured from polarimetric aerosol measurements"
...
(defrule SYNERGY::surface-composition-in-all-spectrum
...
(defrule SYNERGY::pointing-capability
...
(defrule SYNERGY::fire-monitoring-bands
...
(defrule SYNERGY::MW-and-IR-sounders-tropo-sensitivity
...
(defrule SYNERGY::add-all-weather-capability
...
(defrule SYNERGY::add-cloud-mask
...
;; mmw trace gases measurements complement ir measurements of the same parameter with enhanced sensitivity in cirrus
(defrule SYNERGY::add-sensitivity-in-convective-clouds
...
(defrule SYNERGY::column-vs-profile-chemistry-measurements
...
(defrule SYNERGY::tropo-vs-strato-chemistry-measurements
...
; rain rate
(defrule SYNERGY::mmw-sounders-rain-rates-hurricanes
    ...
    (not (REQUIREMENTS::Measurement (Parameter "1.7.3 Rain rate, tropical storms, and hurricanes")
            (taken-by ?ins)))
    =>
    ;(printout t "SYNERGY::mmw-sounders-rain-rates-hurricanes" crlf)
    (duplicate ?cl (Parameter "1.7.3 Rain rate, tropical storms, and hurricanes"))
    )

; sensitivity over oceans
(defrule SYNERGY::sensitivity-over-oceans
...
(defrule SYNERGY::count-num-soundings-per-day
...
Occultation Events for Orbit Selection for Global/Regional Observation, RAST 2007"
    ...
    ?m <- (REQUIREMENTS::Measurement (Parameter "1.3.3 GPS radio occultation") (taken-by ?tk)
        (num-soundings-per-day# nil) (num-of-sats-per-plane# ?ns&~nil) )
    => 
...
(defrule SYNERGY::count-num-soundings-per-day-when-nil
    "Computes number of soundings per day from number of satellites carrying 
    GPS receivers, based on paper Research on the Number and Distribution of GPS
Occultation Events for Orbit Selection for Global/Regional Observation, RAST 2007"
    
   ?m <-  (REQUIREMENTS::Measurement (Parameter "1.3.3 GPS radio occultation") (taken-by ?tk)
        (num-soundings-per-day# nil) (num-of-sats-per-plane# nil) )
    => 
...
(defrule SYNERGY::num-soundings-per-day-add
    "Computes number of soundings per day from number of satellites carrying 
    GPS receivers, based on paper Research on the Number and Distribution of GPS
Occultation Events for Orbit Selection for Global/Regional Observation, RAST 2007"
...
;; SMAP specific rules
;(defrule SYNERGY::SMAP-spatial-disaggregation "A frequent coarse spatial resolution measurement can be combined with a sparse high spatial resolution measurement to produce a frequent high spatial resolution measurement with average accuracy"
...
(defrule SYNERGY::clouds-and-radiation2
...
(defrule SYNERGY::clouds-and-radiation3
    "For EOS, it is convenient to express requirements in terms of number of CERES flying"
    ?sub1<- (CAPABILITIES::Manifested-instrument (Intent "Earth radiation budget radiometers") (Name ?n1&~ACRIM))
    ?sub2<- (CAPABILITIES::Manifested-instrument (Intent "Earth radiation budget radiometers") (Name ?n2&~?n1&~ACRIM))
...
(defrule SYNERGY::CERES-sampling-multiple-angles-SW 
...
(defrule SYNERGY::CERES-sampling-multiple-angles-LW
    "For CERES, it is required to have one instrument in a cross-track scanning configuration 
    to get good spatial sampling and another one in azimuth scanning configuration in order to 
    sample all possible angles"
    (declare (no-loop TRUE))
...
(defrule SYNERGY::CERES-sampling-single-angle-SW
    "For CERES, it is required to have one instrument in a cross-track scanning configuration 
    to get good spatial sampling and another one in azimuth scanning configuration in order to 
    sample all possible angles"
    (declare (no-loop TRUE))
...
(defrule SYNERGY::CERES-sampling-single-angle-LW
    "For CERES, it is required to have one instrument in a cross-track scanning configuration 
    to get good spatial sampling and another one in azimuth scanning configuration in order to 
    sample all possible angles"
    (declare (no-loop TRUE))
...
(defrule SYNERGY::CERES-sampling-time-SW2
    "For CERES, it is required to have one instrument in a cross-track scanning configuration 
    to get good spatial sampling and another one in azimuth scanning configuration in order to 
    sample all possible angles"
...
(defrule SYNERGY::CERES-sampling-time-LW2
...
(defrule SYNERGY::CERES-sampling-time-SW3
    "For CERES, it is required to have one instrument in a cross-track scanning configuration 
    to get good spatial sampling and another one in azimuth scanning configuration in order to 
    sample all possible angles"
    (declare (no-loop TRUE) (salience -3))
...
(defrule SYNERGY::CERES-sampling-time-LW3
    "For CERES, error due to sampling time decreases with  the number of independent samples as 
    described in Wielicki et al 1995"
    (declare (no-loop TRUE) (salience -3))
    ?sub <- (REQUIREMENTS::Measurement (Parameter "A4.Clouds and radiation") (num-of-indep-samples# ?n&3))
...
(defrule SYNERGY::radiation-budget-err-total-SW
...
(defrule SYNERGY::radiation-budget-err-total-LW
    "Computes total rms error for a radiation bugdte mission. Has 3 components, instrument error, 
    angular sampling error, and time sampling error. See Wielicki et al 95 about CERES for more info"
    (declare (no-loop TRUE) (salience -5))
...
; 
(defrule SYNERGY::sea-surface-winds-combining-SAR-and-scatterometer
    ...
    (SYNERGY::cross-registered-instruments (instruments $?ins) (degree-of-cross-registration spacecraft))
    (test (member$ ?sar $?ins))
...
(defrule SYNERGY::sea-surface-winds-with-high-winds
...
(defrule SYNERGY::add-multi-angular-capability
...
(defrule SYNERGY::HSR-TR-ThreeD-combination-scheme
...
(defrule SYNERGY::temperature-sounding-suite
...
(defrule SYNERGY::GRAVITY-measurements2 
   ...
   (assert (REQUIREMENTS::Measurement  (Coverage-of-region-of-interest Global) (flies-in ?miss) (Id GRAVITY2) (Instrument GRAVITY) (orbit-altitude# ?h) (orbit-anomaly# ?ano) (Parameter "3.2.6 Ocean mass distribution") (Pointing-capability High) (Region-of-interest Global) (taken-by GRAVITY) (Temporal-resolution Highest-1h-orless) (ThreeD Some-3D-multi-angle) )) 
   (assert (REQUIREMENTS::Measurement  (Coverage-of-region-of-interest Global) (flies-in ?miss) (Id GRAVITY3) (Instrument GRAVITY) (orbit-altitude# ?h) (orbit-anomaly# ?ano) (Parameter "4.1.3 glacier mass balance") (Pointing-capability High) (Region-of-interest Cold-regions) (taken-by GRAVITY) (Temporal-resolution Highest-1h-orless) (ThreeD Some-3D-multi-angle) ))
   (assert (REQUIREMENTS::Measurement  (Coverage-of-region-of-interest Global) (flies-in ?miss) (Id GRAVITY4) (Instrument GRAVITY) (orbit-altitude# ?h) (orbit-anomaly# ?ano) (Parameter "2.7.3 groundwater storage") (Pointing-capability High) (Region-of-interest Global) (taken-by GRAVITY) (Temporal-resolution Highest-1h-orless) (ThreeD Some-3D-multi-angle) ))
   (assert (REQUIREMENTS::Measurement  (Coverage-of-region-of-interest Global) (flies-in ?miss) (Id GRAVITY5) (Instrument GRAVITY) (orbit-altitude# ?h) (orbit-anomaly# ?ano) (Parameter "3.2.2 seafloor topography") (Pointing-capability High) (Region-of-interest Global) (taken-by GRAVITY) (Temporal-resolution Highest-1h-orless) (ThreeD Some-3D-multi-angle) ))
   )

(defrule SYNERGY::fire-monitoring
...
(defrule SYNERGY::CO2-temperature-error
...
(defrule SYNERGY::CO2-pressure-error
...
(defrule SYNERGY::ocean-wind-vector-sensitivity-high-winds
    ...
    =>
    ;(assert (SYNERGY::cross-registered (measurements (insert$ $?m (+ 1 (length$ $?m)) (str-cat ?id1 "-syn-" ?id2 )))))
    ;(printout t winds crlf)
    (duplicate ?Ku (sensitivity-in-high-winds High) (taken-by (str-cat ?ins1 "-syn-" ?ins2)) (Id (str-cat ?id1 "-syn-" ?id2 )))
    )

(defrule SYNERGY::ocean-wind-vector-complete
    "If a Ku-band measurement of ocean winds is combined with a C-band measurement 
...
(defrule SYNERGY::ocean-wind-vector-sensitivity-in-rain
    "If a Ku-band measurement of ocean winds is combined with a C-band measurement 
    then the sensitivity at high speed improves"
    (declare (salience 10) (no-loop TRUE))
...
(defrule SYNERGY::ocean-wind-vector-atmospheric-correction
...
(defrule SYNERGY::cross-instrument-calibration-heritage-SCLP
...
(defrule SYNERGY::several-lidars-for-aerosols
    ...
    (test (> (SameOrBetter Temporal-resolution ?tr1 Very-low-1-3-weeks) 0))
    (test (> (SameOrBetter Temporal-resolution ?tr2 Very-low-1-3-weeks) 0))
    (test (> (SameOrBetter Temporal-resolution ?tr3 Very-low-1-3-weeks) 0))
...
(defrule SYNERGY::AOD-lidar-passive-combination
...
(defrule SYNERGY::NOx-strategy
    ...
    (test (>= (SameOrBetter Vertical-Spatial-Resolution ?vsr2 Medium-200m-2km) 0))
    (SYNERGY::cross-registered (measurements $?m))
    (test (member$ ?id1 $?m))
    (test (member$ ?id2 $?m))
    (test (member$ ?id3 $?m))
...
(defrule SYNERGY::formaldehyde-strategy
...
(defrule SYNERGY::black-carbon-strategy
...
(defrule SYNERGY::atmospheric-humidity-sounding-strategy
...
(defrule SYNERGY::water-vapor-transport-strategy
...
(defrule SYNERGY::column-and-profile-ozone-measurements
