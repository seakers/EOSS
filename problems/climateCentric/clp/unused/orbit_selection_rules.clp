(defrule ORBIT-SELECTION::assert-all-possible-mission-orbits
    (declare (salience 20))
    ?orig <- (MANIFEST::Mission (Name ?name) (select-orbit yes) (in-orbit ?nil) (num-of-sats-per-plane# ?ns) (mission-architecture ?arch) (orbit-altitude# nil) (orbit-type nil) (orbit-RAAN nil) (orbit-inclination nil)  (instruments $?payload))
    =>
        ...
        (foreach ?h (create$ 275 400 600 800 1300)
            (foreach ?i (create$ polar SSO near-polar)
                (foreach ?raan (create$ AM PM DD NA)
                    (if (valid-orbit ?typ ?h ?i ?raan) then (duplicate ?orig (in-orbit (str-cat ?typ "-" ?h "-" ?i "-" ?raan)) (num-of-sats-per-plane# ?ns) (mission-architecture ?arch) (orbit-altitude# ?h) (orbit-type ?typ) (orbit-RAAN ?raan) (orbit-inclination ?i) )))
        )))
    (retract ?orig)
    )
