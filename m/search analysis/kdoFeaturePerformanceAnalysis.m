function [feature_names, feature_stats] = kdoFeaturePerformanceAnalysis()
...
filepath = strcat(path, filesep, 'result', filesep, 'AIAA JAIS', filesep, 'random_noFilter_noCross');
...
try
    ...
    for i=1:length(files)
        m = regexp(files(i).name, '.*aos(?<id>[0-9]*)*', 'names');
        unique_ids.add(m.id);
    end
...
%     iter.next;
    ...
    figure(1)
    n_stages = 3;
    feature_names = cell(n_stages*n_ops,1);
    feature_stats = zeros(n_stages*n_ops,6);%b_sup,b_conf,a_sup,a_conf,select,credit
    counter = 1;
    for stage_i = 0:n_stages - 1
        ...
        mining.FilterApplication.run(label_before.name, feature_file.name, strcat(filepath,filesep,'out.csv'));
        ind_before = logical(csvread('out.csv',1,0)');
        mining.FilterApplication.run(label_after.name, feature_file.name, strcat(filepath,filesep,'out.csv'));
        ind_after = logical(csvread('out.csv',1,0)');
        ...
        for feature_j = 1:n_ops
            feature_names(counter) = rule_string(feature_j);
            
            %compute support and confidence at beginning and end of data mining stage
            feature_stats(counter,1) = sum(and(good_before, ind_before(:,feature_j)))/length(good_before);
            feature_stats(counter,2) = sum(and(good_before, ind_before(:,feature_j)))/sum(ind_before(:,feature_j));
            feature_stats(counter,3) = sum(and(good_after, ind_after(:,feature_j)))/length(good_after);
            feature_stats(counter,4) = sum(and(good_after, ind_after(:,feature_j)))/sum(ind_after(:,feature_j));
            ...
            ind = and(ind_before(:,feature_j),good_before);
            scatter(-data_before(ind,63),data_before(ind,64),20,'b');
            hold off
            axis([0,0.3,0,25000])
            ...
            if(isempty(opData))
                ...
                feature_stats(counter,5) = length(opData(:,1));
                feature_stats(counter,6) = sum(opData(:,2));
            end
            counter = counter + 1;
        end
    end
