...
filepath = strcat(path, filesep, 'result');%, filesep, 'AIAA JAIS', filesep, 'tmp');
...
try
    ...
    for i=1:length(files)
        m = regexp(files(i).name, '.*(?<id>[0-9]*)*', 'names');
        unique_ids.add(m.id);
    end
    
    feature_names = cell(n_stages*n_ops,unique_ids.size());
    feature_stats = zeros(n_stages*n_ops,6,unique_ids.size());%b_sup,b_conf,a_sup,a_conf,select,credit
...
%         fclose(fid);
        ...
        for stage_i = 0:n_stages - 1
            ...
            for feature_j = 1:n_ops
                ...
                feature_stats(counter,1,file_i) = sum(and(good_before, ind_before(:,feature_j)))/length(good_before);
                feature_stats(counter,2,file_i) = sum(and(good_before, ind_before(:,feature_j)))/sum(ind_before(:,feature_j));
                feature_stats(counter,3,file_i) = sum(and(good_after, ind_after(:,feature_j)))/length(good_after);
                feature_stats(counter,4,file_i) = sum(and(good_after, ind_after(:,feature_j)))/sum(ind_after(:,feature_j));
...
%                 else
%                     feature_stats(counter,5,file_i) = length(opData(:,1));
%                     feature_stats(counter,6,file_i) = sum(opData(:,2));
%                 end
                counter = counter + 1;
            end
        end
