function creditAnalysisEOSS(mode)

path = '/Users/nozomihitomi/Dropbox/EOSS/problems/climateCentric/';

origin = cd(path);
% ops = {'OnePointCrossover+BitFlip','OnePointCrossover+KnowledgeARMPoor+BitFlip', 'OnePointCrossover+KnowledgeARMGood+BitFlip'};
ops = {'OnePointCrossover+BitFlip','OnePointCrossover+KnowledgeHumanPoor+BitFlip', 'OnePointCrossover+KnowledgeHumanGood+BitFlip'};
nops = length(ops);
labels = {'1pt','poor', 'good'};
close all
switch mode
...
%             fileType =strcat('*.creditcsv');
            ...
            save(strcat(tmp{1},'_credit.mat'),'allcredits');
    ...
    case 2 %analyze the files. mode 1 already assumed run
        ...
        h = waitbar(filesProcessed/nFiles,'Processing files...');
        for a=1:nFiles
            newMatFileName = strsplit(matfiles(a).name,'.');
            filesProcessed = filesProcessed + 1;
            waitbar(filesProcessed/nFiles);
            load(matfiles(a).name);
...
%                             normEraCreditOneOp = (rawEraCredits.get(operator)-minCredit)/(maxCredit-minCredit);
            ...
            plot(cred);
            save(strcat(newMatFileName{1},'_credit','.mat'),'cred','labels','allcredits');
            legend(labels)
            xlabel('Epoch')
...
%             title(strcat(problemName{a},'  ',creditDef{c},' velocity'))
            ...
            subplot(1,nFiles,filesProcessed)
            area(sel);
            save(strcat(newMatFileName{1},'_sel','.mat'),'sel','labels');
